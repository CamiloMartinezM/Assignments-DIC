...
\begin{homeworkProblem}{(Ambrosio–Tortorelli Approximation)}
    ...
    \begin{adjustwidth}{2.5em}{0pt}
        ...
        \[
            \begin{split}
                \frac{u_{i,j}^{k+1} - u_{i,j}^k}{\tau} &= \left[2 v_{i,j}^k \left[\frac{(v_{i+1,j}^k - v_{i-1,j}^k)(u_{i+1,j}^k - u_{i-1,j}^k)}{2h_x} + \frac{(v_{i,j+1}^k - v_{i,j-1}^k)(u_{i,j+1}^k - u_{i,j-1}^k)}{2h_y}\right] + (v_{i,j}^k)^2 \Delta u^k\right] - \dots \\ 
        ...
        \[
            A^k = \frac{(v_{i+1,j}^k - v_{i-1,j}^k)(u_{i+1,j}^k - u_{i-1,j}^k)}{2h_x} + \frac{(v_{i,j+1}^k - v_{i,j-1}^k)(u_{i,j+1}^k - u_{i,j-1}^k)}{2h_y}
        \]
        \[
