...
\begin{homeworkProblem}{(Ambrosio–Tortorelli Approximation)}
    ...
    \begin{adjustwidth}{2.5em}{0pt}
        ...
        \[
            \begin{split}
                \frac{u_{i,j}^{k+1} - u_{i,j}^k}{\tau} &= A^k(u_{i,j}^k) u_{i,j}^k - \beta (u_{i,j}^{k+1} - f_{i,j}) \\
                \rightarrow u_{i,j}^{k+1} (1 + \beta \tau) &= \tau \left[A^k(u_{i,j}^k) u_{i,j}^k + \beta f_{i,j}\right] + u_{i,j}^k \\
                \rightarrow u_{i,j}^{k+1} &= \frac{\tau}{1 + \beta \tau} \left[A^k(u_{i,j}^k) u_{i,j}^k + \beta f_{i,j}\right] + \frac{1}{1 + \beta \tau} u_{i,j}^k
            \end{split}
        \]

        where $u = (u_1, \ldots, u_N)^T$, and the $N \times N \text{ matrix } A(u) = (a_{k,\ell}(u))$ is a space discretization of the term $\text{div}(v^2 \nabla u)$ that satisfies
        \[
            a_{k,\ell} := 
            \begin{cases}[r]
                \frac{(v_{k}^2 + v_{\ell}^2)}{2h^2} & (\ell \in N(k)), \\
                -\sum\limits_{n \in N(k)} \frac{(v_{k}^2 + v_{n}^2)}{2h^2} & (\ell = k), \\
                0 & (\text{else}).
            \end{cases} 
        \]

        Here, \( N(k) \) are the neighbors of pixel \( k \), and \( h \) is the distance between pixels (assuming we took $h_1 = h_2 = h$ in both $x$ and $y$ directions to simplify the equations above).\\
        ...
        \[
            \Delta v_{i,j}^k = \frac{v_{i+1,j}^{k} - 2v_{i,j}^{k} + v_{i-1,j}^{k}}{h^2} + \frac{v_{i,j+1}^{k} - 2v_{i,j}^{k} + v_{i,j-1}^{k}}{h^2}
        \]
        \[
            \lvert \nabla u_{i,j}^k \rvert^2 = \frac{(u_{i+1,j}^k - u_{i-1,j}^k)^2 + (u_{i,j+1}^k - u_{i,j-1}^k)^2}{4h^2}
        \]
