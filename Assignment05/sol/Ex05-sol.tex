...
Since the area grows 4 times by size and the spatial grid size remains 1, T will have to be 4 times larger as well thus $T'=992$ and since the grey value range scaled down by a factor of 255, the contrast paramter $\lambda$ which usually represents a quantile of the histogram of image gradient magnitude will be scaled down by 255 as well: $\lambda' = 0.016$ and for the smoothing parameter $\sigma$ decreases by half to achieve the same convolution result: $\sigma' = 0.5$.
    ...
    We found from the lectures the computational effort becomes 16 times more which results in 198.4 seconds
...
\subsection*{(a)}
We know that the diffusion process can be seen as a energy functional minimisation. Thus we can calculate the smoothness term for Charbonnier ($\Psi_{cb}$) and TV flow ($\Psi_{tv}$).
$$\Psi_{cb} = 2\lambda(\sqrt{\lambda^2 + |\nabla u|^2})$$
$$\Psi_{tv} = 2\sqrt{|\nabla u|^2}$$
we know that in the energy functional, the $\alpha$ in front of the smoothness term can be seen as the stopping time. To achieve the same result when $\lambda$ is small, the stopping time of TV flow should be $\lambda$ times the stopping time of Charbonnier
\subsection*{(b)}
See attached code file
\subsection*{(c)}
We find the optimal $\lambda$ when the correlation is minimal with least iterations. For acros.pgm we get $\lambda = 0.1$ with $corr = 0.029$ and for pepper-noise we get $\lambda = 4$ with $corr = 0.063$
\end{homeworkProblem}
